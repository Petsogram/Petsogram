...
const oldDiscoverStart = `function DiscoverPage() {`;
const oldDiscoverEnd = `export default App;`;

const originalCode = code.substring(code.indexOf(oldDiscoverStart), code.indexOf(oldDiscoverEnd));

// Rather than replacing the whole file by string matching, I'll replace the DiscoverPage function.
// Since DiscoverPage and ServicesPage are large, I'll extract them using regex or substring indexing.
const discoverStartIdx = code.indexOf('function DiscoverPage() {');
const servicesStartIdx = code.indexOf('function ServicesPage(', discoverStartIdx);
const myHistoryStartIdx = code.indexOf('function MyHistoryPage(', servicesStartIdx);

if (discoverStartIdx !== -1 && servicesStartIdx !== -1 && myHistoryStartIdx !== -1) {
  ...
  const requestGPS = (forceReal = false) => {
    ...
    if (demoMode && !forceReal) {
      // DY Patil University, Nerul coordinates
...
`;
  ...
  const newCode = code.substring(0, discoverStartIdx) + newDiscoverCode + '\n' + newServicesCode + '\n' + code.substring(myHistoryStartIdx);
