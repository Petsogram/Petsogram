...
// Find the START of the LAST ProfilePage in the file (to skip any inner corrupted ones)
const profileIdx = content.lastIndexOf('function ProfilePage({ setPage }) {');

if (profileIdx === -1) {
  console.log("Could not find ProfilePage");
  process.exit(1);
}

// Slice off everything from ProfilePage onwards
let cleanContent = content.substring(0, profileIdx);

// Append a completely fresh ProfilePage, SettingsPage, and PetsogramApp
const freshEnd = \`
...
// Fix missing escapes in template literal before writing
const unescapedEnd = freshEnd.replace(/\\\\\\\`/g, '\`').replace(/\\\\\$/g, '$');

fs.writeFileSync(appFile, cleanContent + unescapedEnd);
console.log("Successfully restored and rebuilt end of App.jsx");
